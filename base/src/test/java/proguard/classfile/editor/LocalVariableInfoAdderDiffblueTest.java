...
public class LocalVariableInfoAdderDiffblueTest {
  /**
   * Method under test:
   * {@link LocalVariableInfoAdder#LocalVariableInfoAdder(ProgramClass, LocalVariableTableAttribute)}
   */
  @Test
  public void testNewLocalVariableInfoAdder() {
    // Arrange
    ProgramClass targetClass = new ProgramClass();
    LocalVariableTableAttribute targetLocalVariableTableAttribute = new LocalVariableTableAttribute(1, 3,
        new LocalVariableInfo[]{new LocalVariableInfo(1, 3, 1, 1, 1)});

    // Act
    LocalVariableInfoAdder actualLocalVariableInfoAdder = new LocalVariableInfoAdder(targetClass,
        targetLocalVariableTableAttribute);
    LibraryClass clazz = new LibraryClass();
    LibraryMethod method = new LibraryMethod(1, "Name", "Descriptor");

    CodeAttribute codeAttribute = new CodeAttribute(1);
    actualLocalVariableInfoAdder.visitLocalVariableInfo(clazz, method, codeAttribute,
        new LocalVariableInfo(1, 3, 1, 1, 1));

    // Assert
    LocalVariableInfo[] localVariableInfoArray = targetLocalVariableTableAttribute.localVariableTable;
